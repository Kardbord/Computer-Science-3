...
#ifndef MAX_SKEW_HEAP_HPP
#define MAX_SKEW_HEAP_HPP
...
public:
	...
	//Default Constructor
	MaxSkewHeap() :root(nullptr), heapSize(0), mergesDone(0) {}

	//Destructor
	~MaxSkewHeap() { clearHeap(); }
	...
	MaxSkewHeap<Comparable> merge(MaxSkewHeap<Comparable> & heap2); //TODO define merge with a seperate heap (for fun); heapSize += heap2.getHeapSize();
...
template<typename Comparable>
void MaxSkewHeap<Comparable>::insert(Comparable newData) {
	if (!root) {
		++heapSize;
...
template<typename Comparable>
std::shared_ptr<HeapNode<Comparable>> MaxSkewHeap<Comparable>::merge(std::shared_ptr<HeapNode<Comparable>> & h1, std::shared_ptr<HeapNode<Comparable>> & h2) {
...
template<typename Comparable>
void MaxSkewHeap<Comparable>::swap(std::shared_ptr<HeapNode<Comparable>> & node1, std::shared_ptr<HeapNode<Comparable>> & node2) {
	auto temp = node1;
	node1 = node2;
	node2 = temp;
}
...
template<typename Comparable>
std::string MaxSkewHeap<Comparable>::toString(std::string & indent, std::shared_ptr<HeapNode<Comparable>> const & node, bool isLeft) {
	if (!node) return "";
...
template<typename Comparable>
Comparable MaxSkewHeap<Comparable>::popMax() {
	assert(root);
	auto max = root->data;
	--heapSize;
